...
#             print('Scanning') #Disable Dormant: Scanning for cat
    ...
    if limopen.value() == 0  and stuck==0 and sink==0:
        ...
        motmos.value(0)
        solmos.value(0)
    if pir2inside.value() == 0 and opend == 1 and pir1outside.value() == 0:
        time.sleep(4)
...
# #                 #b=(sum(cat2)-sum(data))creates a zero condition for cat2 probably not necessary
            if (data) in catdata:#checks if the rfid data maches the data of the stored cat 1
                    ...
                    time.sleep(4)
                    if limopen.value()==1:
                        print('door stuck open')
                        speaker.on(G4)
                        led.on()
                        time.sleep(0.5)
                        led.off()
                        print('door stuck activated')#indicates if the chip is good. opening door sequence should follow
# #                   
            else:
